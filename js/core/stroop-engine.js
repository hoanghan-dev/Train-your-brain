 ...
 * Thử thách chọn màu mực thực tế, ức chế phản xạ đọc chữ, đếm ngược từng câu
 */
...
class StroopGame {
    ...
    nextQuestion() {
        ...
        // Chọn ngẫu nhiên từ và màu chữ bất đối xứng (75% incongruent)
    ...
    handleAnswer(selectedColorKey) {
        ...
        // Quy tắc cốt lõi: Chọn màu mực thực tế của từ
        ...
        if (isCorrect) {
            ...
            this.streak = 0; // Ngắt streak khi sai
            if (feedbackEl) {
    ...
    handleTimeout() {
        ...
        const feedbackEl = document.getElementById('stroop-feedback');
        if (feedbackEl) {
            feedbackEl.textContent = 'Hết giờ câu này! Chuỗi streak đã bị ngắt.';
            feedbackEl.className = 'badge bg-danger bg-opacity-25 text-rose border border-danger border-opacity-50 p-2 text-sm';
        }
        setTimeout(() => {
            this.nextQuestion();
    ...
    completeGame() {
        ...
        if (player.challenges && player.challenges.stroop) {
            player.challenges.stroop.totalPlays = (player.challenges.stroop.totalPlays || 0) + 1;
